        ...
        loop
            ...
            try {
                ...
                if (TIMER.Elapsed() > 9) {
                    ...
                    printf("[METERS] %f PPS | %f WPS | Largest Share %f | Difficulty %f | Height = %u | Balance: %f NXS | Payout: %f NXS | %02d:%02d:%02d ", PPS, WPS, nLargestShare / 10000000.0, nDifficulty / 10000000.0, nBestHeight, nAccountBalance / 1000000.0, nCurrentPayout / 1000000.0, (SecondsElapsed / 3600) % 60, (SecondsElapsed / 60) % 60, (SecondsElapsed) % 60);
                    ...
                    /** send curl request */
                    curl = curl_easy_init();
                    std::string url = IP + "/workers/api/";

                    std::string strArguments = "pps=" + boost::lexical_cast<std::string>(PPS)
                            + "&wps=" + boost::lexical_cast<std::string>(WPS)
                            + "&address=" + ADDRESS
                            + "&miner=" + MINERNAME;
                ...
                else if (PACKET.HEADER == CLIENT->ACCOUNT_BALANCE) {
                    nAccountBalance = bytes2uint64(PACKET.DATA);
                }
                    /** Set the Current Pending Payout if Message Received. **/
                else if (PACKET.HEADER == CLIENT->PENDING_PAYOUT) {
                    nCurrentPayout = bytes2uint64(PACKET.DATA);
                }
...
int main(int argc, char *argv[]) {
    ...
    if (MINERNAME.empty()) {
        printf("You need to specify miner name! Make sure it's unique");
        sleep(1000);
        return 0;
    }
    ...
    if(argc > 5)
	nThreads = boost::lexical_cast<int>(argv[5]);
    ...
    printf("Your miner name: %s *!\n" , MINERNAME.c_str());

    Core::InitializePrimes();
    nStartTimer = (unsigned int) time(0);

    Core::ServerConnection MINERS(IP, PORT, MINERNAME, nThreads, nTimeout);
