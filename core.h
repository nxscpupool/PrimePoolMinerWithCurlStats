...
namespace LLP {
    ...
    public:
        ...
        inline void GetBalance() {
            this -> WritePacket(GetPacket(GET_BALANCE));
        }

        /** Get the Current Pending Payouts for the Next Coinbase Tx. **/
        inline void GetPayouts() {
            this -> WritePacket(GetPacket(GET_PAYOUT));
        }
...
namespace Core {
    ...
    public:
        ...
        ServerConnection(std::string ip, std::string port, std::string minerName, int nMaxThreads, int nMaxTimeout) : IP(ip), PORT(port), MINERNAME(minerName), TIMER(), nThreads(nMaxThreads), nTimeout(nMaxTimeout), THREAD(boost::bind(&ServerConnection::ServerThread, this)) {
